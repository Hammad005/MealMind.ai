...
const SharedByShared = ({ open, setOpen, id }) => {
  const { shareSharedRecipe, sendedRecipes, shareRecipeLoading } =
    useSharedStore();
  ...
  const handleShareRecipe = (receiverId) => {
    shareSharedRecipe(receiverId, id);
  };
  return (
    <>
        ...
        <DialogContent className={"max-h-[80vh] overflow-y-auto p-4"}>
          <DialogHeader>
            <DialogTitle className={"flex gap-1 items-end"}>
              <Share className="w-5 h-5 text-primary" /> Share This Recipe
            </DialogTitle>
